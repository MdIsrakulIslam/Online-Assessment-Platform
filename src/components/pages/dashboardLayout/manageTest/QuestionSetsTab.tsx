        ...
        <div className="space-y-6">
          {fields.map((field, index) => (
            <div key={field.id} className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm relative">
              ...
              <p className="text-gray-800 font-semibold mb-4 whitespace-pre-wrap">{field.title}</p>

              {field.type !== "text" && field.options && (
                <div className="space-y-3 pl-4">
                  {field.options.map((opt, oIndex) => (
                    <div key={oIndex} className="flex items-center gap-3">
                      <div className={`w-5 h-5 flex items-center justify-center border ${field.type === 'radio' ? 'rounded-full' : 'rounded'} ${opt.isCorrect ? 'border-green-500 bg-green-50' : 'border-gray-300'}`}>
                         {opt.isCorrect && <div className={`bg-green-500 ${field.type === 'radio' ? 'w-2 h-2 rounded-full' : 'w-full h-full'}`}></div>}
                      </div>
                      <span className={`text-gray-700 ${opt.isCorrect ? 'font-medium' : ''}`}>{String.fromCharCode(65 + oIndex)}. {opt.text}</span>
                      {opt.isCorrect && <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto" />}
                    </div>
                  ))}
                </div>
              )}
              ...
              {field.type === "text" && field.textAnswer && (
                <div className="mt-4 p-4 bg-gray-50 rounded italic text-gray-600 border border-gray-100">
